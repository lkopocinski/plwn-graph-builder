...
class PLWN:
    ...
    def lexical_units(self) -> Dict[int, LexicalUnit]:
        ...
        lexical_units = {row.lu_id: LexicalUnit(**row) for row in results}
        results.close()

        return lexical_units

    def synsets(self) -> Dict[int, Set[LexicalUnit]]:
        query = "SELECT DISTINCT id AS synset_id FROM synset;"

        results = self._db.execute(query)
        synsets_indices = [row.synset_id for row in results]
        synsets_lexical_units = [
            self.synset_lexical_units(synset_id=idx) for idx in synsets_indices
        ]
        synsets = {
            synset_id: lu_set
    ...
    def synset_lexical_units(self, synset_id: int) -> Set[LexicalUnit]:
        ...
        results = self._db.execute(query)
        lexical_units = {row.lexical_unit_id for row in results}
        results.close()
